//
//  FollowingSearchPaginationViewModel.swift
//  Clique
//
...
@Observable class FollowingSearchPaginationViewModel: PaginationViewModel {
    ...
    init(uid: String, userStore: UserStore) {
        ...
        self.fetchFunction = { input in
            let users = try await UserService.searchUserFollowing(.init(path: .init(userId: uid), query: .init(search: input.query, page: input.page, size: input.size)))
...
extension FollowingSearchPaginationViewModel {
    func reset() {
        items = []
        page = 0
        done = false
